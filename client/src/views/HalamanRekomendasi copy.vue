                ...
                <table class="w-full text-left border-collapse">
                  ...
                  <tbody class="divide-y divide-surface-100 text-sm">
                    <tr v-for="(dosen, index) in hasilRekomendasi" :key="index"
                      ...
                      <!-- <td class="p-4">
                        <div class="font-bold text-surface-800">{{ dosen.NAMA }}</div>
                        <div class="text-xs text-surface-400 font-mono mt-1 truncate max-w-50">{{ dosen.PROGRAM_STUDI }}
                        </div>

                        <div class="mt-3 space-y-1.5">
                          <div class="text-[10px] flex items-start gap-2">
                            <span
                              class="bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded border border-blue-100 font-bold shrink-0">Match
                              Leksikal</span>
                            <span class="text-surface-600 font-mono italic">{{ dosen['Irisan Kata (Lexical)'] }}</span>
                          </div>
                          <div class="text-[10px] flex items-start gap-2">
                            <span
                              class="bg-fuchsia-50 text-fuchsia-600 px-1.5 py-0.5 rounded border border-fuchsia-100 font-bold shrink-0">Fokus
                              Semantik</span>
                            <span class="text-surface-600 font-mono italic">{{ dosen['Frasa Terkait (KeyBERT)']
                              }}</span>
                          </div>
                        </div>
                      </td> -->
...
const pantauStatusServer = () => {
  ...
  const interval = setInterval(async () => {
    try {
      ...
      serverMessage.value = "Flask Server belum menyala...";
    }
  }, 1500);
};
