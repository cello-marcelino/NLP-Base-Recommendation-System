  ...
  <div class="max-w-7xl mx-auto py-8">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
      <div>
        ...
        <p class="text-slate-500 mt-1 font-medium">Visualisasi Metode dan Komputasi Teknis Sistem Rekomendasi
          menggunakan model SBERT & BM25</p>
      </div>
      <button @click="handleRefresh" :disabled="isRefreshing"
        class="group flex items-center gap-2 bg-white hover:bg-violet-50 border border-slate-200 hover:border-violet-200 text-slate-600 hover:text-violet-700 px-4 py-2 rounded-xl text-sm font-bold shadow-sm transition-all active:scale-95">
        <svg
          :class="['w-4 h-4', isRefreshing ? 'animate-spin text-violet-600' : 'group-hover:rotate-180 transition-transform duration-500']"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div class="lg:col-span-4 space-y-6">
          ...
          <div class="space-y-5 relative z-10">
            <div>
              ...
              <input v-model="judulInput" type="text" placeholder="Ketik rencana judul..."
                class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-violet-50 focus:border-violet-400 outline-none transition-all text-sm font-medium">
            </div>
            <div>
              ...
              <textarea v-model="abstrakInput" rows="5" placeholder="Jelaskan secara singkat masalah dan metode..."
                class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-violet-50 focus:border-violet-400 outline-none transition-all text-sm resize-none"></textarea>
            </div>
            <div class="bg-slate-50 border border-slate-100 p-4 rounded-xl">
              <div class="flex justify-between items-center mb-2">
                <label class="block text-sm font-bold text-slate-700">Distribusi Bobot AI</label>
                <span class="text-[10px] font-mono font-bold bg-violet-100 text-violet-700 px-2 py-0.5 rounded">
                  L:{{ bobotLexical }}% | S:{{ 100 - bobotLexical }}%
                </span>
              </div>
              <input type="range" v-model="bobotLexical" min="0" max="100" step="10"
                class="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-violet-600">
              <div class="flex justify-between text-[10px] font-bold text-slate-400 uppercase mt-1.5">
            ...
            <div class="flex gap-3">
              <div class="w-1/3">
                ...
                <input v-model="kRank" type="number" min="1" max="20"
                  class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-center font-bold text-slate-700 outline-none focus:border-violet-400">
              </div>
              <button @click="jalankanPipeline" :disabled="isProcessing"
                class="w-2/3 bg-violet-600 hover:bg-violet-700 disabled:bg-slate-300 disabled:text-slate-500 text-white font-bold rounded-xl transition-all active:scale-95 shadow-sm shadow-violet-200 mt-6 relative overflow-hidden">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          ...
          <div class="w-full bg-slate-100 rounded-full h-2 mb-3 overflow-hidden">
            <div
              :class="['h-2 rounded-full transition-all duration-500 ease-out', progressPct === 100 ? 'bg-emerald-500' : 'bg-violet-500']"
      ...
      <div class="lg:col-span-8 space-y-4">
        ...
        <div v-if="!hasStarted"
          class="h-full min-h-400px flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50 text-slate-400">
        ...
        <div v-for="step in pipeline" :key="step.id" v-show="hasStarted && step.status !== 'idle'"
          ...
          <div v-if="step.status === 'running'" class="absolute top-0 left-0 h-0.5 bg-violet-500 animate-pulse"
            style="width: 100%;"></div>
          ...
          <div @click="step.open = !step.open"
            ...
            <div class="flex items-center gap-4">
              <div
                :class="['w-10 h-10 rounded-xl flex items-center justify-center text-lg font-bold transition-all', step.status === 'running' ? 'bg-violet-100 text-violet-600 animate-pulse' : 'bg-slate-100 text-slate-500']">
            ...
            <span
              :class="['text-[10px] font-bold px-3 py-1 rounded-lg uppercase tracking-wider', step.status === 'running' ? 'bg-violet-100 text-violet-700' : 'bg-emerald-50 text-emerald-600']">
              {{ step.status }}
            </span>
          </div>
          ...
          <div v-show="step.open" class="p-5 border-t border-slate-100 bg-slate-50/30">
            ...
            <div v-if="step.id === 1" class="space-y-4">
              <div class="bg-white border border-slate-200 p-5 rounded-xl shadow-sm">
                ...
                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 text-center">Alur
                ...
                <div
                  ...
                  <div
                    class="px-4 py-2.5 bg-violet-50 border border-violet-100 text-violet-700 rounded-lg w-full md:w-auto shadow-sm">
                ...
                <div class="border-t border-slate-100 pt-5">
                  <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Ringkasan Parameter
                    Terkirim</div>
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col gap-1.5">
                      <div class="flex items-center gap-2 text-[11px] font-bold text-slate-500 uppercase">
                        <div class="w-5 h-5 rounded bg-violet-100 text-violet-600 flex items-center justify-center"><svg
                    ...
                    <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col gap-1.5">
                      ...
                      <div class="text-sm font-bold text-slate-800">Menyaring Top <span class="text-emerald-600">{{
            ...
            <div v-if="step.id === 2" class="bg-white border border-slate-200 p-4 rounded-xl shadow-sm">
              <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Hasil Pemotongan Token
                (Front-end view)</div>
              <p class="text-xs text-slate-500 mb-3 font-medium">Teks diubah menjadi huruf kecil (lowercasing), tanda
              ...
              <div class="flex flex-wrap gap-2 bg-slate-50 p-4 rounded-lg border border-slate-100">
                <span v-for="token in sampleTokens" :key="token"
                  class="bg-white text-slate-600 border border-slate-200 shadow-sm px-2.5 py-1.5 rounded-md text-xs font-semibold">
                  {{ token }}
                </span>
              </div>
            </div>
            ...
            <div v-if="step.id === 4" class="space-y-4">
              <div class="bg-white border border-slate-700 p-5 rounded-xl text-center shadow-inner">
                <div class="text-violet-700 text-xs font-bold uppercase tracking-widest mb-2">Formula Penggabungan Bobot
                </div>
                ...
                <table class="w-full text-left text-xs border-collapse">
                  <thead class="bg-slate-50 text-slate-500 font-bold uppercase tracking-wider">
                    <tr>
                      ...
                      <th class="p-3 border-b border-slate-200 text-center text-blue-600">Lexical ({{ bobotLexical }}%)
                      </th>
                      <th class="p-3 border-b border-slate-200 text-center text-fuchsia-600">Semantic ({{ 100 -
                        bobotLexical }}%)</th>
                  ...
                  <tbody class="divide-y divide-slate-100 font-mono">
                    <tr v-for="dosen in hasilRekomendasi.slice(0, 3)" :key="dosen.NAMA">
                      ...
                      <td class="p-3 text-center font-bold text-violet-700 bg-violet-50/50">{{ dosen['Hybrid Score'] }}
                      </td>
                    </tr>
                  </tbody>
                </table>
                <div
                  class="p-2 text-center text-[10px] font-bold text-slate-400 bg-slate-50/80 uppercase tracking-widest border-t border-slate-100">
            ...
            <div v-if="step.id === 5 && hasilRekomendasi.length > 0">
              <div class="bg-violet-50 border border-violet-100 p-5 rounded-xl mb-5 flex items-start gap-4 shadow-sm">
                ...
                <div>
                  <div class="text-xs font-bold text-violet-400 uppercase tracking-widest mb-1">Rekomendasi Utama (Rank
                    1)</div>
                  <div class="font-extrabold text-violet-900 text-lg">{{ hasilRekomendasi[0].NAMA }}</div>
                  <div class="text-sm text-violet-700 mt-1 font-medium">{{
                  ...
                  <tbody class="divide-y divide-slate-100 text-sm">
                    <tr v-for="(dosen, index) in hasilRekomendasi" :key="index"
                      ...
                      <td class="p-4 text-center">
                        <span
                          class="bg-slate-100 text-slate-600 font-extrabold px-3 py-1.5 rounded-lg text-xs shadow-sm">{{
                          index + 1 }}</span>
                      </td>
                      <td class="p-4">
                      ...
                      <td class="p-4 text-center">
                        <span
                          class="text-violet-600 font-extrabold bg-violet-50 border border-violet-100 px-3 py-1 rounded-lg">{{
                          dosen['Hybrid Score'] }}</span>
                      </td>
                      <td class="p-4 text-center">
                        <button @click="bukaDetail(dosen)"
                          class="text-xs font-bold bg-white border border-slate-300 text-slate-600 hover:border-violet-500 hover:bg-violet-50 hover:text-violet-700 px-4 py-2 rounded-lg transition-all shadow-sm">
                          Riwayat
                        </button>
                      </td>
                    </tr>
