<template>
  <div class="max-w-7xl mx-auto py-8">
    <div class="flex justify-between items-end mb-8">
      ...
      <div class="bg-white border border-surface-200 px-4 py-2 rounded-xl shadow-sm text-sm font-semibold text-surface-700">
        Total: <span class="text-primary-600">{{ daftarDosen.length }}</span> Dosen
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-surface-200 overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr
              class="bg-surface-50/50 border-b border-surface-200 text-xs uppercase tracking-widest text-surface-400 font-bold">
          ...
          <tbody class="divide-y divide-surface-100">
            <tr v-for="(dosen, index) in daftarDosen" :key="index" class="hover:bg-surface-50/80 transition-colors group">
              <td class="p-5">
              ...
              <td class="p-5 max-w-md">
                <div class="flex flex-wrap gap-1.5">
                  <span v-for="(skill, i) in formatArray(dosen.BIDANG_KEAHLIAN).slice(0, 4)" :key="i"
                    class="bg-surface-100 text-surface-600 px-2.5 py-1 rounded-md text-xs font-medium border border-surface-200">
                    {{ skill }}
                  </span>
                  <span v-if="formatArray(dosen.BIDANG_KEAHLIAN).length > 4"
                    class="bg-primary-50 text-primary-600 px-2 py-1 rounded-md text-xs font-bold">
              ...
              <td class="p-5 text-center">
                <button @click="bukaDetail(dosen)"
                  class="text-sm bg-white border border-surface-300 text-surface-600 font-semibold px-4 py-2 rounded-lg transition-all group-hover:border-primary-400 group-hover:text-primary-700 shadow-sm hover:shadow">
                  Lihat Detail
                </button>
              </td>
            </tr>
