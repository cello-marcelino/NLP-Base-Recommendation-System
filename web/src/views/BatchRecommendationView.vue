...
const submitExcel = async () => {
  if (!file.value) {
    recStore.batchError = 'Pilih file Excel terlebih dahulu'
    return
  }
  ...
  await recStore.uploadExcelBatch(formData)
}

const downloadJson = () => {
  if (recStore.batchResults.length === 0) return
  const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(recStore.batchResults, null, 2))
  const downloadAnchor = document.createElement('a')
  downloadAnchor.setAttribute("href", dataStr)
  downloadAnchor.setAttribute("download", `siredo_batch_results_${new Date().toISOString().slice(0,10)}.json`)
        ...
        <div class="bp-card-content">
          ...
          <div class="bp-excel-area">
            ...
            >
              ...
              <p class="upload-hint">Maksimal 100 proposal per proses batch.</p>
            </div>
            ...
            <button 
              @click="submitExcel"
              :disabled="!file || recStore.isBatchProcessing"
              class="bp-submit-btn"
            >
              <svg v-if="recStore.isBatchProcessing" class="ic-spin" fill="none" viewBox="0 0 24 24">
                <circle class="op25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                <path fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
              </svg>
              {{ recStore.isBatchProcessing ? 'Memproses Batch...' : 'Mulai Proses Batch' }}
            </button>
          </div>
          
          <p v-if="recStore.batchError" class="bp-error mt-4 text-center">⚠ {{ recStore.batchError }}</p>
        </div>
      </div>
      
      <!-- Results Section -->
      <div v-if="recStore.batchResults.length > 0" class="bp-card bp-results-card">
        <div class="bp-results-header">
          <h3 class="bp-results-title">Hasil Pemrosesan Batch ({{ recStore.batchResults.length }} Item)</h3>
          <button @click="downloadJson" class="bp-download-btn">Download JSON</button>
        </div>
        ...
        <div class="bp-table-wrap">
          <table class="bp-table">
            ...
            <tbody>
              <tr v-for="res in recStore.batchResults" :key="res.id" class="bp-tr">
                <td class="bp-td">
