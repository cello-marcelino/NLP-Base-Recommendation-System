...
const config = ref({ threshold: 0.0, adaptive_alpha_threshold: 15, is_adaptive: true, manual_alpha: 0.7 })
...
const fetchConfig = async () => {
  try {
    ...
    message.value = 'Gagal memuat konfigurasi dari server'
    isError.value = true
  } finally {
...
const saveConfig = async () => {
  ...
  isError.value = false
  try {
    await api.patch('/config', config.value)
    message.value = 'Konfigurasi berhasil disimpan.'
    setTimeout(() => message.value = '', 3000)
  } catch {
    message.value = 'Gagal menyimpan konfigurasi.'
    isError.value = true
  } finally {
      ...
      <form v-else @submit.prevent="saveConfig" class="config-form">
        ...
        <div class="config-field">
          <div class="cf-header">
            ...
            <span class="cf-value">{{ Number(config.threshold).toFixed(1) }}</span>
          </div>
          <div class="slider-wrap">
            <span class="slider-tick">0.0</span>
            <input type="range" v-model.number="config.threshold" min="0" max="5" step="0.1" class="cf-range">
        ...
        <div class="config-field">
          <div class="cf-header">
            ...
            <button
              type="button"
              class="ic-toggle"
              :class="{ 'ic-toggle--on': config.is_adaptive }"
          ...
          <div v-if="config.is_adaptive" class="adaptive-settings">
            <div class="config-field" style="margin-top:1rem;">
              <div class="cf-header">
                ...
                <div class="cf-number-wrap">
                  <input type="number" v-model.number="config.adaptive_alpha_threshold" min="5" max="50" class="cf-number">
              ...
              <div class="mode-preview">
                <div class="mode-card mode-keyword">
                  ...
                  <span class="mode-detail">&lt; {{ config.adaptive_alpha_threshold }} token → α=70% BM25</span>
                </div>
                <div class="mode-sep">↔</div>
                <div class="mode-card mode-abstrak">
                  <span class="mode-label">📄 Abstrak Mode</span>
                  <span class="mode-detail">≥ {{ config.adaptive_alpha_threshold }} token → β=65% SBERT</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-else class="manual-settings">
            <div class="cf-header" style="margin-top:1rem; margin-bottom:0.5rem">
              ...
              <span class="cf-value">{{ Math.round(config.manual_alpha * 100) }}%</span>
            </div>
            <div class="slider-wrap">
              <span class="slider-tick">0%</span>
              <input type="range" v-model.number="config.manual_alpha" min="0" max="1" step="0.05" class="cf-range">
              <span class="slider-tick">100%</span>
            </div>
            <p class="cf-desc" style="margin-top:0.5rem">Sisa bobot ({{ Math.round((1 - config.manual_alpha) * 100) }}%) akan dialokasikan untuk SBERT.</p>
          </div>
        </div>
        ...
        <div class="config-footer">
          <p v-if="message" :class="['cf-message', isError ? 'cf-error' : 'cf-success']">{{ message }}</p>
          <button type="submit" :disabled="isSaving" class="btn-save">
            <svg v-if="isSaving" class="spin-icon" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
...
.slider-marks { position: relative; height: 16px; margin-top: 4px; padding: 0 2.75rem; }
.slider-marks span { position: absolute; font-size: 0.65rem; color: var(--text-muted); transform: translateX(-50%); }
